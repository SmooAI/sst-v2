...
export interface RDSv2Props {
     ...
     */
    engine: 'mysql5.6' | 'mysql5.7' | 'mysql8.0' | 'postgresql13.12' | 'postgresql13.9' | 'postgresql14.10' | 'postgresql15.5' | 'postgresql16.1';
...
export type RDSv2EngineType = RDSv2Props['engine'];

export interface RDSv2CdkServerlessClusterProps extends Omit<DatabaseClusterProps, 'vpc' | 'engine' | 'defaultDatabaseName' | 'scaling'> {
    vpc?: IVpc;
}
...
/**
 * The `RDS` construct is a higher level CDK construct that makes it easy to create an [RDS Serverless v2 Cluster](https://aws.amazon.com/rds/).
 *
 * @example
 *
 * ```js
 * import { RDSv2 } from "sst/constructs";
 *
 * new RDSv2(stack, "Database", {
 *   engine: "postgresql13.12",
...
export class RDSv2 extends Construct implements SSTConstruct {
    ...
    public getBindings(): BindingProps {
        return {
            ...
            permissions: {
                ...
                'secretsmanager:GetSecretValue': [this.secret.secretFullArn || `${this.secret.secretArn}*`],
                'secretsmanager:DescribeSecret': [this.secret.secretFullArn || `${this.secret.secretArn}*`],
    ...
    private validateCDKPropWhenIsClusterProps() {
        const { cdk } = this.props;
        const props = (cdk?.cluster || {}) as RDSv2CdkServerlessClusterProps;
    ...
    private validateMigrationsFileExists(migrations: string) {
        if (!fs.existsSync(migrations)) throw new Error(`Cannot find the migrations in "${path.resolve(migrations)}".`);
    }

    private getEngine(engine: RDSv2EngineType): IClusterEngine {
        if (engine === 'mysql5.6') {
        ...
        throw new Error(
            `The specified "engine" is not supported for sst.RDS. Only mysql5.6, mysql5.7, postgresql13.12, and postgresql13.9 engines are currently supported.`,
        );
    }

    private getScaling(scaling?: RDSv2Props['scaling']): Pick<DatabaseClusterProps, 'serverlessV2MinCapacity' | 'serverlessV2MaxCapacity'> {
        return {
            serverlessV2MinCapacity: AuroraCapacityUnit[scaling?.minCapacity || 'ACU_2'],
            serverlessV2MaxCapacity: AuroraCapacityUnit[scaling?.maxCapacity || 'ACU_16'],
        };
    }

    private getVpc(props: RDSv2CdkServerlessClusterProps): IVpc {
        if (props.vpc) {
            return props.vpc;
        }
    ...
    private getVpcSubnets(props: RDSv2CdkServerlessClusterProps): SubnetSelection | undefined {
        if (props.vpc) {
    ...
    private createCluster() {
        ...
        const app = this.node.root as App;
        const clusterProps = (cdk?.cluster || {}) as RDSv2CdkServerlessClusterProps;

        const name = app.logicalPrefixedName(this.node.id);
        const serverlessV2WriterName = `${name}-serverless-v2-writer`;
        ...
        return new DatabaseCluster(this, 'Cluster', {
            ...
            writer: ClusterInstance.serverlessV2(serverlessV2WriterName),
    ...
    private importCluster() {
        const { cdk } = this.props;
        return cdk!.cluster as DatabaseCluster;
    }
